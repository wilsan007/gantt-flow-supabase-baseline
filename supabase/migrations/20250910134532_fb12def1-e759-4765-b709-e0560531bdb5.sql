-- Module RH MVP - Tables essentielles (correction)
...
-- Fonction pour calculer les jours ouvrés entre deux dates (correction du nom de variable)
...
-- Storage bucket pour les documents employés
...
CREATE POLICY "Employees can upload their own documents" ON storage.objects 
...
CREATE POLICY "HR can manage all tenant employee documents" ON storage.objects 
FOR ALL USING (
  bucket_id = 'employee-documents' 
  AND auth.uid() IS NOT NULL
);
