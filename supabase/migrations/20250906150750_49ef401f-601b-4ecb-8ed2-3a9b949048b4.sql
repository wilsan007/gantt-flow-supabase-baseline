...
DECLARE
    ...
    task5_id UUID;
BEGIN
    -- Get user IDs (users already exist from previous migration)
    SELECT id INTO marie_id FROM public.profiles WHERE full_name = 'Marie Dupont' LIMIT 1;
    SELECT id INTO jean_id FROM public.profiles WHERE full_name = 'Jean Martin' LIMIT 1;
    SELECT id INTO sophie_id FROM public.profiles WHERE full_name = 'Sophie Bernard' LIMIT 1;
    SELECT id INTO pierre_id FROM public.profiles WHERE full_name = 'Pierre Dubois' LIMIT 1;
    SELECT id INTO camille_id FROM public.profiles WHERE full_name = 'Camille Laurent' LIMIT 1;
    SELECT id INTO lucas_id FROM public.profiles WHERE full_name = 'Lucas Moreau' LIMIT 1;
