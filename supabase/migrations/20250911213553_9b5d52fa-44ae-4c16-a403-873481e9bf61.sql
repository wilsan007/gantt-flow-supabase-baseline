-- Migration corrigée pour nettoyer et optimiser les alertes HR
...
-- Associer les autres catégories importantes
INSERT INTO alert_type_solutions (alert_type_id, solution_id, priority_order, context_conditions)
SELECT 
  at.id, 
  as_sol.id, 
...
FROM alert_types at
CROSS JOIN alert_solutions as_sol
WHERE at.category IN ('compliance', 'cost', 'engagement') 
  AND as_sol.category = at.category
  AND NOT EXISTS (
    SELECT 1 FROM alert_type_solutions ats 
    WHERE ats.alert_type_id = at.id AND ats.solution_id = as_sol.id
  );
