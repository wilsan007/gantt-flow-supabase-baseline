-- Créer un index pour optimiser les requêtes de validation (seulement si pas déjà existant)
CREATE INDEX IF NOT EXISTS idx_task_actions_task_id_weight ON public.task_actions(task_id, weight_percentage);
...
-- Créer le trigger pour valider la somme des poids (supprimer d'abord s'il existe)
DROP TRIGGER IF EXISTS validate_task_actions_weight_trigger ON public.task_actions;
