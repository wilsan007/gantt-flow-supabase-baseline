              ...
              <Card key={process.id} className="hover:shadow-lg transition-shadow">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <Badge className={getStatusColor(process.status)}>
                      {process.status === 'in_progress' && <Clock className="h-3 w-3 mr-1" />}
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-sm">Début: {new Date(process.start_date).toLocaleDateString()}</span>
                  </div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              Responsable: {task.responsible} • Échéance: {new Date(task.due_date).toLocaleDateString()}
                            </p>
                          </div>
                        </div>
            ...
            {offboardingProcesses.map((process) => {
              ...
              <Card key={process.id} className="hover:shadow-lg transition-shadow">
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center gap-4">
                    <Calendar className="h-4 w-4 text-muted-foreground" />
                    <span className="text-sm">Dernier jour: {new Date(process.last_work_day).toLocaleDateString()}</span>
                  </div>
