-- Fix security issue: Remove SECURITY DEFINER and add proper RLS
...
-- Enable RLS on the view (this will use the underlying tables' RLS policies)
-- Since this is a view, RLS is enforced through the underlying tables
-- The view will respect the RLS policies of employees, tasks, departments, etc.

-- Add a comment to document the security approach
COMMENT ON VIEW current_alerts_view IS 'View for computed alerts. Security is enforced through RLS policies on underlying tables (employees, tasks, departments, etc.)';