...
comment_data AS (
  INSERT INTO public.task_comments (task_id, content, author_id, comment_type, tenant_id)
  SELECT 
    t.id,
    'Commentaire de suivi sur cette tâche - ' || t.title,
    (SELECT profile_id FROM user_data ORDER BY random() LIMIT 1),
    'general',
    get_user_tenant_id()
  FROM task_data t
  RETURNING task_id
...
leave_data AS (
  INSERT INTO public.leave_requests (employee_id, absence_type_id, start_date, end_date, total_days, reason, status, tenant_id)
  SELECT 
    u.profile_id,
    (SELECT id FROM public.absence_types LIMIT 1),
...
attendance_data AS (
  INSERT INTO public.attendances (employee_id, date, check_in, check_out, break_duration, status, tenant_id)
  SELECT 
    u.profile_id,
    CURRENT_DATE - INTERVAL '20 days' - (ROW_NUMBER() OVER() || ' days')::interval,
    '09:00',
    '17:30',
    60,
    'present',
...
INSERT INTO public.evaluations (employee_id, employee_name, evaluator_id, evaluator_name, type, period, status, overall_score, tenant_id)
SELECT 
  u.profile_id,
  u.full_name,
  (SELECT profile_id FROM user_data WHERE profile_id != u.profile_id LIMIT 1),
  'Manager Évaluateur',
  'annual',
  '2024',
  'completed',
