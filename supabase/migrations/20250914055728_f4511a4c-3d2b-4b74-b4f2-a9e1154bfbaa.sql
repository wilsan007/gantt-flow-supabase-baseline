-- Insertion des nouveaux types d'alertes pour la gestion de projet (en évitant les doublons)
...
-- Alertes additionnelles pour équilibrer avec RH
('TASK_OVERFLOW', 'Débordement de tâches', 'Accumulation excessive de tâches en cours', 'medium', 'Workflow', 'project', '{"conditions": [{"field": "active_tasks_count", "operator": ">", "value": 50}]}'),
('DEPENDENCY_BOTTLENECK', 'Goulot de dépendances', 'Blocages dus aux dépendances entre tâches', 'high', 'Workflow', 'project', '{"conditions": [{"field": "dependency_chain_length", "operator": ">", "value": 5}]}');
...
('Simplification du workflow', 'Simplifier les processus de travail complexes', 'Workflow', 85, 'low', 'short_term', '["Cartographier processus actuels", "Identifier complexités", "Proposer simplifications", "Tester nouvelles méthodes"]', '["process_manager", "team_lead"]'),
('Gestion des dépendances', 'Optimiser la gestion des dépendances entre tâches', 'Planning', 80, 'medium', 'immediate', '["Mapper dépendances", "Identifier chemins critiques", "Réorganiser planning", "Communiquer contraintes"]', '["project_manager", "team_lead"]');