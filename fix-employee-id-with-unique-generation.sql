-- Solution finale : générer un employee_id vraiment unique globalement
-- au lieu de se baser sur le tenant qui peut créer des conflits
...
BEGIN
  ...
  -- 9. Générer employee_id GLOBALEMENT UNIQUE (pas par tenant)
  debug_log := debug_log || '9️⃣ Génération employee_id globalement unique...' || E'\n';
  
  -- Utiliser un compteur global pour éviter les conflits
  ...
  -- 10. Créer employé avec DELETE + INSERT pour éviter les conflits
  debug_log := debug_log || '🔟 Création employé (DELETE + INSERT)...' || E'\n';
  
  BEGIN
    -- Supprimer d'abord tout employé existant pour ce user_id
    DELETE FROM public.employees WHERE user_id = user_record.id;
    debug_log := debug_log || '🗑️ Employés existants supprimés pour ce user_id' || E'\n';
...
COMMENT ON FUNCTION debug_tenant_creation IS 'Version finale avec génération globale employee_id et DELETE+INSERT pour éviter conflits';
