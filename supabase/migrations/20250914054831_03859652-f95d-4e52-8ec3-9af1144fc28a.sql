-- Ajouter une colonne application_domain aux tables d'alertes
ALTER TABLE alert_types 
ADD COLUMN application_domain TEXT NOT NULL DEFAULT 'hr';

ALTER TABLE alert_instances 
ADD COLUMN application_domain TEXT NOT NULL DEFAULT 'hr';

-- Mettre à jour les types d'alertes existants avec le bon domaine d'application
UPDATE alert_types SET application_domain = 'project' 
WHERE code IN ('DEADLINE_RISK', 'PERFORMANCE_DROP');

UPDATE alert_types SET application_domain = 'hr' 
WHERE code IN (
  'OVERLOAD_90', 'WORKLOAD_HIGH', 'HIGH_UTILIZATION_ABOVE_AVG', 
  'UNDERUTILIZATION', 'ABSENCE_PATTERN', 'UNUSED_LEAVE', 
  'NO_EVALUATION', 'VACATION_OVERDUE', 'SUCCESSION_RISK', 
  'DOCUMENT_MISSING', 'SALARY_BUDGET_EXCEEDED', 'ABSENCE_SPIKE',
  'CAPACITY_MISMATCH', 'OBJECTIVES_OVERDUE', 'LEAVE_BALANCE_LOW'
);

-- Recréer la vue avec la colonne application_domain
DROP VIEW IF EXISTS current_alerts_view;

CREATE OR REPLACE VIEW current_alerts_view AS

-- 1. OVERLOAD_90 - Surcharge critique ≥90% (HR)
SELECT 
...
-- 2. WORKLOAD_HIGH - Surcharge de travail (>15 tâches) (HR)
SELECT 
  'workload-' || e.id as id,
...
-- 3. HIGH_UTILIZATION_ABOVE_AVG - Utilisation élevée vs moyenne (+25%) (HR)
SELECT 
...
-- 4. UNDERUTILIZATION - Sous-utilisation <30% (HR)
SELECT 
...
-- 5. ABSENCE_PATTERN - Pattern d'absences anormal (HR)
SELECT 
...
-- 6. DEADLINE_RISK - Tâches en retard (PROJECT)
SELECT 
...
-- 7. PERFORMANCE_DROP - Tâches bloquées (PROJECT)
SELECT 
...
-- 8. UNUSED_LEAVE - Congés non utilisés (HR)
SELECT 
...
-- 9. NO_EVALUATION - Pas d'évaluation depuis 12 mois (HR)
SELECT 
...
-- 10. VACATION_OVERDUE - Congés en retard (HR)
SELECT 
...
-- 11. SUCCESSION_RISK - Risque de succession (HR)
SELECT 
...
-- 12. DOCUMENT_MISSING - Documents employés manquants (HR)
SELECT 
...
-- 13. SALARY_BUDGET_EXCEEDED - Budget salaire dépassé (HR)
SELECT 
...
-- 14. ABSENCE_SPIKE - Pic d'absences (HR)
SELECT 
...
-- 15. CAPACITY_MISMATCH - Décalage capacité (HR)
SELECT 
...
-- 16. OBJECTIVES_OVERDUE - Objectifs en retard (HR)
SELECT 
...
-- 17. LEAVE_BALANCE_LOW - Solde congés bas (HR)
SELECT 
...
-- Ajouter un commentaire pour documenter l'approche de sécurité et le domaine d'application
COMMENT ON VIEW current_alerts_view IS 'Vue pour les alertes calculées avec domaine d''application (hr/project). La sécurité est appliquée via les politiques RLS des tables sous-jacentes.';