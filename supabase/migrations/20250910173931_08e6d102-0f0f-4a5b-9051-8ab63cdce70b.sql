...
-- 1. D'abord, s'assurer que la table employees a un index unique sur user_id
...
BEGIN
  ...
  UPDATE public.absences SET employee_name = NEW.full_name WHERE employee_id = NEW.user_id;
  UPDATE public.attendances SET employee_name = NEW.full_name WHERE employee_id = NEW.user_id;
