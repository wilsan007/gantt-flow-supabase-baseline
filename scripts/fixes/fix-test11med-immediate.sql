-- Correction immédiate pour test11med@yahoo.com
-- Utilisateur connecté mais profil manquant (HTTP 406)

-- 1. Vérifier l'état actuel de l'utilisateur
SELECT 
  '=== UTILISATEUR TEST11MED ===' as section,
  id as user_id,
  email,
  email_confirmed_at,
  created_at
FROM auth.users 
WHERE email = 'test11med@yahoo.com';

-- 2. Vérifier s'il y a une invitation
SELECT 
  '=== INVITATION TEST11MED ===' as section,
  id,
  email,
  full_name,
  tenant_id,
...
INSERT INTO public.invitations (
  ...
  'Test Med User',
  now() + interval '7 days',
  '{"company_name": "Test Med Company"}'::jsonb
)
...
BEGIN
  ...
  IF user_record.id IS NULL THEN
    RAISE NOTICE 'ERREUR: Utilisateur test11med@yahoo.com non trouvé';
    RETURN;
  END IF;
...
SELECT 
  ...
  CASE 
    WHEN EXISTS (SELECT 1 FROM public.profiles WHERE email = 'test11med@yahoo.com') THEN '✅ Profil créé'
    ELSE '❌ Profil manquant'
  END as profil_status,
  CASE 
    WHEN EXISTS (SELECT 1 FROM public.employees WHERE email = 'test11med@yahoo.com') THEN '✅ Employé créé'
