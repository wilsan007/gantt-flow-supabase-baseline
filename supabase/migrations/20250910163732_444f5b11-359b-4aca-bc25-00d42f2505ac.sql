...
-- 2. Créer la table des absences réelles si elle n'existe pas
DO $$
BEGIN
  IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'absences') THEN
...
-- 3. Créer la table des retards si elle n'existe pas
DO $$
BEGIN
  IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'tardiness') THEN
    CREATE TABLE public.tardiness (
      id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
      employee_id uuid NOT NULL,
      date date NOT NULL,
...
-- 4. Créer les politiques RLS pour les absences si elles n'existent pas
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'absences' AND policyname = 'Users can manage tenant absences'
  ) THEN
    CREATE POLICY "Users can manage tenant absences" ON public.absences
      FOR ALL USING (auth.uid() IS NOT NULL AND tenant_id = get_user_tenant_id());
  END IF;
END
$$;

-- 5. Créer les politiques RLS pour les retards si elles n'existent pas
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'tardiness' AND policyname = 'Users can manage tenant tardiness'
  ) THEN
    CREATE POLICY "Users can manage tenant tardiness" ON public.tardiness
      FOR ALL USING (auth.uid() IS NOT NULL AND tenant_id = get_user_tenant_id());
  END IF;
END
$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE event_object_table = 'absences' AND trigger_name = 'update_absences_updated_at'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.triggers 
    WHERE event_object_table = 'tardiness' AND trigger_name = 'update_tardiness_updated_at'
  ) THEN
...
-- 7. Insérer des données de test pour les absences
...
-- 8. Insérer des données de test pour les retards
WITH employee_data AS (
  SELECT id, full_name FROM public.employees WHERE tenant_id = get_user_tenant_id() LIMIT 6
)
