...
const REASON_CONFIG: Record<AccessDeniedReason, {
  ...
  module_restricted: {
    ...
    description: (props) => `Le module "${props.module || 'ce module'}" est réservé aux utilisateurs autorisés.`,
  ...
  no_data: {
    ...
    description: (props) => `Aucune donnée n'est disponible dans le module "${props.module || 'ce module'}".`,
        ...
        <CardContent className="space-y-6">
          ...
          {currentRole && (
            <div className="bg-muted p-4 rounded-lg">
              <p className="text-sm font-medium mb-2">Informations sur votre compte :</p>
              <div className="space-y-1 text-sm text-muted-foreground">
                <p>• <strong>Rôle actuel :</strong> {currentRole}</p>
                {requiredRole && <p>• <strong>Rôle requis :</strong> {requiredRole}</p>}
                {requiredPermission && <p>• <strong>Permission requise :</strong> {requiredPermission}</p>}
              </div>
            </div>
          )}
          ...
          <p className="text-xs text-center text-muted-foreground">
            Ces restrictions sont en place pour protéger les données sensibles de votre organisation.
          </p>
        </CardContent>
      </Card>
    </div>
 ...
 */
export const AccessDeniedInline: React.FC<Pick<AccessDeniedProps, 'reason' | 'module' | 'requiredRole'>> = ({
  reason,
  module,
  requiredRole
}) => {
