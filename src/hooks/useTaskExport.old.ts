/**
 * 🎯 useTaskExport - Hook d'Export Excel/PDF pour Tâches
 * Pattern: Notion, Linear, Monday.com
 * 
 * Fonctionnalités:
 * - Export Excel (.xlsx) avec formatage
 * - Export PDF avec mise en page professionnelle
 * - Support des tâches filtrées
 * - Métadonnées (date export, utilisateur, filtres appliqués)
 */
...
export const useTaskExport = () => {
  ...
  const prepareExportData = useCallback((tasks: Task[]) => {
    return tasks.map((task) => ({
      ...
      'Assigné à': typeof task.assignee === 'string' 
        ? task.assignee 
    ...
    try {
      ...
      if (includeMetadata) {
        ...
        if (filters) {
          ...
          if (filters.status.length > 0) {
            doc.text(`Statuts: ${filters.status.map(s => STATUS_LABELS[s] || s).join(', ')}`, 14, yPos);
            yPos += 5;
          }
          if (filters.priority.length > 0) {
            doc.text(`Priorités: ${filters.priority.map(p => PRIORITY_LABELS[p] || p).join(', ')}`, 14, yPos);
            yPos += 5;
          }
        }
