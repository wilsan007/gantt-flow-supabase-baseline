-- Add simple test data based on existing structure

-- Update profiles table to have employee-like data if records exist
UPDATE public.profiles 
SET 
  job_title = CASE 
    WHEN id = (SELECT id FROM public.profiles ORDER BY created_at LIMIT 1) THEN 'Directrice RH'
    WHEN id = (SELECT id FROM public.profiles ORDER BY created_at OFFSET 1 LIMIT 1) THEN 'Développeur Senior'
    ELSE 'Employé'
  END,
  contract_type = 'CDI',
  employee_id = 'EMP' || LPAD((ROW_NUMBER() OVER (ORDER BY created_at))::text, 3, '0'),
