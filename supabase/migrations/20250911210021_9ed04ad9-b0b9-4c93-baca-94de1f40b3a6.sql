-- Insérer les types d'alertes manquants
INSERT INTO public.alert_types (code, name, description, category, severity, auto_trigger_conditions) 
SELECT code, name, description, category, severity, auto_trigger_conditions::jsonb
FROM (VALUES
  ...
  ('TRAINING_OVERDUE_2', 'Formation obligatoire en retard', 'Formation obligatoire non complétée dans les délais', 'training', 'high', '{"overdue_mandatory_training": {"operator": ">", "value": 0}}'),
  ('SKILL_GAP_DETECT', 'Écart de compétences détecté', 'Compétences requises non maîtrisées', 'training', 'medium', '{"skill_gap_percentage": {"operator": ">", "value": 30}}'),
  ...
  ('STRESS_INDICATORS_2', 'Indicateurs de stress élevés', 'Signaux de stress ou burnout détectés', 'safety', 'high', '{"stress_score": {"operator": ">", "value": 70}}'),
...
) AS new_alerts(code, name, description, category, severity, auto_trigger_conditions)
WHERE NOT EXISTS (
  SELECT 1 FROM public.alert_types WHERE alert_types.code = new_alerts.code
);

-- Insérer les solutions prédéfinies si elles n'existent pas
INSERT INTO public.alert_solutions (title, description, category, cost_level, implementation_time, effectiveness_score, required_roles, action_steps) 
SELECT title, description, category, cost_level, implementation_time, effectiveness_score, required_roles, action_steps::jsonb
FROM (VALUES
...
) AS new_solutions(title, description, category, cost_level, implementation_time, effectiveness_score, required_roles, action_steps)
WHERE NOT EXISTS (
  SELECT 1 FROM public.alert_solutions WHERE alert_solutions.title = new_solutions.title
);