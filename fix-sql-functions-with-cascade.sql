-- =====================================================
-- CORRECTION DES FONCTIONS SQL AVEC GESTION DES DÉPENDANCES
-- À exécuter dans Supabase Dashboard > SQL Editor
-- =====================================================

-- 1. SAUVEGARDER LES POLITIQUES EXISTANTES
-- =====================================================
-- Nous allons recréer is_super_admin avec une signature différente pour éviter les conflits

-- 2. CRÉER UNE NOUVELLE VERSION DE is_super_admin
-- =====================================================
CREATE OR REPLACE FUNCTION public.is_super_admin_v2(check_user_id uuid)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- 3. METTRE À JOUR LES POLITIQUES POUR UTILISER LA NOUVELLE FONCTION
-- =====================================================

-- Politique pour roles
DROP POLICY IF EXISTS "Super admin write access for roles" ON roles;
CREATE POLICY "Super admin write access for roles" ON roles
  FOR ALL USING (is_super_admin_v2(auth.uid()));

-- Politique pour permissions
DROP POLICY IF EXISTS "Super admin write access for permissions" ON permissions;
CREATE POLICY "Super admin write access for permissions" ON permissions
  FOR ALL USING (is_super_admin_v2(auth.uid()));

-- Politique pour role_permissions
DROP POLICY IF EXISTS "Super admin write access for role_permissions" ON role_permissions;
CREATE POLICY "Super admin write access for role_permissions" ON role_permissions
  FOR ALL USING (is_super_admin_v2(auth.uid()));

-- Politique pour absence_types
DROP POLICY IF EXISTS "Super admin write access for absence_types" ON absence_types;
CREATE POLICY "Super admin write access for absence_types" ON absence_types
  FOR ALL USING (is_super_admin_v2(auth.uid()));

-- Politique pour alert_types
DROP POLICY IF EXISTS "Super admin write access for alert_types" ON alert_types;
CREATE POLICY "Super admin write access for alert_types" ON alert_types
  FOR ALL USING (is_super_admin_v2(auth.uid()));

-- Politique pour evaluation_categories
DROP POLICY IF EXISTS "Super admin write access for evaluation_categories" ON evaluation_categories;
CREATE POLICY "Super admin write access for evaluation_categories" ON evaluation_categories
  FOR ALL USING (is_super_admin_v2(auth.uid()));

-- Politique pour expense_categories
DROP POLICY IF EXISTS "Super admin write access for expense_categories" ON expense_categories;
CREATE POLICY "Super admin write access for expense_categories" ON expense_categories
  FOR ALL USING (is_super_admin_v2(auth.uid()));

-- Politique pour skills
DROP POLICY IF EXISTS "Super admin write access for skills" ON skills;
CREATE POLICY "Super admin write access for skills" ON skills
  FOR ALL USING (is_super_admin_v2(auth.uid()));

-- Politique pour positions
DROP POLICY IF EXISTS "Super admin write access for positions" ON positions;
CREATE POLICY "Super admin write access for positions" ON positions
  FOR ALL USING (is_super_admin_v2(auth.uid()));

-- Politique pour alert_type_solutions
DROP POLICY IF EXISTS "Super admin write access for alert_type_solutions" ON alert_type_solutions;
CREATE POLICY "Super admin write access for alert_type_solutions" ON alert_type_solutions
  FOR ALL USING (is_super_admin_v2(auth.uid()));

-- 4. MAINTENANT SUPPRIMER L'ANCIENNE FONCTION ET RECRÉER
-- =====================================================
DROP FUNCTION IF EXISTS public.is_super_admin(uuid) CASCADE;

-- Recréer la fonction avec le bon nom
...
-- 5. REMETTRE LES POLITIQUES AVEC LA FONCTION ORIGINALE
-- =====================================================
...
-- =====================================================
DROP FUNCTION IF EXISTS public.is_super_admin_v2(uuid);

-- 7. CRÉER LES AUTRES FONCTIONS MANQUANTES
-- =====================================================

-- Supprimer les autres fonctions (elles n'ont pas de dépendances)
...
BEGIN
  ...
  v_result := json_build_object(
    ...
    'recommendations', CASE 
      WHEN v_functions_count < 5 THEN 'Exécutez fix-sql-functions-with-cascade.sql'
...
-- =====================================================
-- FIN DE LA CORRECTION AVEC CASCADE
-- =====================================================
